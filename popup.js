...
async function updateSettings (init) {
  ...
  document.getElementById('currentSettings').innerText =
    `Configured battery level: ${(await getKeyFromStorage('triggerLevel')) || 'not set'}%\n` +
    `Configured battery level (fullscreen warning): ${(await getKeyFromStorage('warning_triggerLevel')) || 'not set'}%\n`
